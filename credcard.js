...
function idInvalidCardCompanies(nestedArr) {
  const companies = [];
    let firstDigit = nestedArr.map(([i])=> i);
console.log(firstDigit);
  
   for (let v = 0; v < nestedArr.length; v++) {
    switch(firstDigit[v]) {
  case 3:
    companies.push('Amex');
    break;
  ...
  for (let k = 0; k < companies.length; k++) {
    if ((companies[j] === companies[k]) && j !== k) {
      companies.splice(k, 1);
    }
  }
};
  return companies;
...
function insertCreditCardNumber(num) {
  let credCard = [];
  let numArr = num.split(' ');
  for (let i = 0; i < numArr[0].length; i++) {
    credCard.push(numArr[0][i]);
  }
  return credCard.map(Number);
}; 

console.log(insertCreditCardNumber('453967790801680'));
